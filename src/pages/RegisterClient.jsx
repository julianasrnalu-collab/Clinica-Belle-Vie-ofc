...
export default function RegisterClient() {
  ...
  const handleRegister = async (e) => {
    ...
    setErrorMsg('');
    try {
      await register(formData, 'client');
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12 mt-16">
        ...
        >
          ...
          <div className="text-center mb-8">
            <h1 className="font-serif text-3xl font-bold text-gray-900 mb-2">Novo Cadastro</h1>
            <p className="text-gray-500">Crie sua conta para agendar procedimentos</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="full_name" required value={formData.full_name} onChange={handleChange} className="mt-1.5 bg-gray-50" placeholder="Maria Silva" />
              </div>
              <div>
            ...
            <div className="pt-4 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="email" type="email" required value={formData.email} onChange={handleChange} className="mt-1.5 bg-gray-50" placeholder="maria@email.com" />
              </div>
              <div>
                <Label htmlFor="password">Senha</Label>
                <Input id="password" type="password" required minLength={6} value={formData.password} onChange={handleChange} className="mt-1.5 bg-gray-50" placeholder="Mínimo 6 caracteres" />
              </div>
            </div>

            <Button type="submit" disabled={loading} className="w-full h-12 bg-primary hover:bg-primary/90 text-white rounded-xl text-base font-semibold shadow-md transition-all mt-6">
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><UserPlus className="w-4 h-4 mr-2" /> Criar Minha Conta</>}
            </Button>
          </form>
        </motion.div>
      </div>
